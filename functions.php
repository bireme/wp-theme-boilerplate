...
function wp_theme_boilerplate_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'wp-theme-boilerplate' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'wp_theme_boilerplate_custom_background_args',
			array(
...
function wp_theme_boilerplate_scripts() {
	wp_enqueue_style( 'wp-theme-boilerplate-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'wp-theme-boilerplate-style', 'rtl', 'replace' );

	wp_enqueue_script( 'wp-theme-boilerplate-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
